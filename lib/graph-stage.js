...
import {DebugPan} from './mind-ui/debug-pan';
...
export class GraphStage extends EventEmitter {
  constructor(canvas, options) {
    ...
    this.mindEditManager = new MindEditManager();

    // this.debugPan = new DebugPan(this.container);
  ...
  pinch(e) {
    // this.debugPan.showObj(e);
    if(this.editModel === "waterfall") {
      return;
    }
    let scalePosition = e.center;
    if(this.oldPinchScale) {
      let newScale = (e.scale - this.oldPinchScale);
      this.canvasRender.scale *= (newScale + 1);
    }

    this.oldPinchScale = e.scale;
    ...
    if (targetTap) {
      ...
      this.nodeSelectMenu.on("menuCommand", (command) => {
        ...
        if(command === "edit") {
          ...
          //   position: position
          // });
          this.editPan = new Input(this.container, position, targetTap);
