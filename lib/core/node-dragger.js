...
export class NodeDragger {
    ...
    dragging(position) {
        ...
        deltaPos.x =  deltaPos.x / this.graphStage.canvasRender.scale;
        deltaPos.y =  deltaPos.y / this.graphStage.canvasRender.scale;
        ...
        let martrix = {
            x: this.graphStage.canvasRender.translate.x,
            y: this.graphStage.canvasRender.translate.y,
            k: this.graphStage.canvasRender.scale
        }

        let dragPositionInCanvas = this.graphStage.viewPoint2GlobalPoint(position.x, position.y, martrix) 
        // 从拖拽的这个节点开始计算布局
        // if()
    ...
    dragEnd(position) {
        ...
        if(this.nearBefore && this.draggingTarget.parent) {
            ...
            startRelayout.reLayout(startRelayout);
            this.graphStage.rootMindNode.render(this.graphStage.canvasRender);
        }
