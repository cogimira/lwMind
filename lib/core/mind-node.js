export class MindNode {
  ...
  calLayout(mindNode, steps) {
      let startRect = {};
    if(steps === 1) {
        startRect = Object.assign({}, mindNode.showObjRect);
    }
    // let totalMarginHeight = mindNode.getChildrenMarginHeight();
    let totalMarginHeight = mindNode.marginHeight;
    ...
    for(let i = 0; i < mindNode.children.length; i++) {
        ...
        child.x = startX;
        child.showObjRect.y = startY + offsetHeight + child.marginHeight / 2 - child.height / 2;
  ...
  click(x, y) {
    ...
    if(deltaVector.x > 0 && deltaVector.x < showObjRect.width && deltaVector.y > 0 && deltaVector.y < showObjRect.height) {
        console.log(this);
